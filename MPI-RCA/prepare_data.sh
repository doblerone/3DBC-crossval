...
##########################################################################################################################
## For MPI-ESM_LR driven RCA --> Adjust for more models
########################################################
...
cdo -r -splitmon /lustre/storeB/project/KSS/proj/scenariodata/Precipitation/netcdf/MPI_RCA/hist/hist_MPI_RCA_RR_daily_1971-2010.nc hist_MPI_RCA_RR_daily_1971-2010_month

# Split into daily files
for file in hist_MPI_RCA_RR_daily_1971-2010_month??.nc
do cdo splitday $file ${file}_day
done

# Remove monthly files
rm hist_MPI_RCA_RR_daily_1971-2010_month??.nc

# Rename files
rename "s/.nc_day//g" hist_MPI_RCA_RR_daily_1971-2010_month??.nc_day??.nc
rename "s/month//g" hist_MPI_RCA_RR_daily_1971-2010_month????.nc
...
cdo -r -splitmon /lustre/storeB/project/KSS/proj/scenariodata/Temperature/netcdf/MPI_RCA/hist/hist_MPI_RCA_TM_daily_1971-2010.nc hist_MPI_RCA_TM_daily_1971-2010_month

# Split into daily files
for file in hist_MPI_RCA_TM_daily_1971-2010_month??.nc
do cdo splitday $file ${file}_day
done

# Remove monthly files
rm hist_MPI_RCA_TM_daily_1971-2010_month??.nc

# Rename files
rename "s/.nc_day//g" hist_MPI_RCA_TM_daily_1971-2010_month??.nc_day??.nc
rename "s/month//g" hist_MPI_RCA_TM_daily_1971-2010_month????.nc
...
###################
### Split yearly files into monthly files
cd /lustre/storeB/project/KSS/proj/scenariodata/Precipitation/netcdf/MPI_RCA/rcp85/
for file in rcp85_MPI_RCA_RR_daily_2???_v4.nc
...
for m in 01 02 03 04 05 06 07 08 09 10 11 12
do cdo -r mergetime rcp85_MPI_RCA_RR_daily_2???_v4.nc_month${m}.nc rcp85_MPI_RCA_RR_daily_2011-2100_${m}.nc
done

# Remove single monthly files
rm rcp85_MPI_RCA_RR_daily_2???_v4.nc_month??.nc

# Split into daily files
for file in rcp85_MPI_RCA_RR_daily_2011-2100_??.nc
do cdo splitday $file ${file}_day
done

# Remove merged monthly files
rm rcp85_MPI_RCA_RR_daily_2011-2100_??.nc

# Rename files
rename "s/.nc_day//g" rcp85_MPI_RCA_RR_daily_2011-2100_??.nc_day??.nc

# Copy current and future climate files (to be adjusted by the 3DBC R-scripts)
module load nco #load newest nco version

cd /lustre/storeB/users/andreasd/postclim/3DBC/pr/Cur/
for file in hist_MPI_RCA_RR_daily_1971-2010_????.nc
...
###################
### Split yearly files into monthly files
cd /lustre/storeB/project/KSS/proj/scenariodata/Temperature/netcdf/MPI_RCA/rcp85/
for file in rcp85_MPI_RCA_TM_daily_2???_v4.nc
...
for m in 01 02 03 04 05 06 07 08 09 10 11 12
do cdo -r mergetime rcp85_MPI_RCA_TM_daily_2???_v4.nc_month${m}.nc rcp85_MPI_RCA_TM_daily_2011-2100_${m}.nc
done

# Remove single monthly files
rm rcp85_MPI_RCA_TM_daily_2???_v4.nc_month??.nc

# Split into daily files
for file in rcp85_MPI_RCA_TM_daily_2011-2100_??.nc
do cdo splitday $file ${file}_day
done

# Remove merged monthly files
rm rcp85_MPI_RCA_TM_daily_2011-2100_??.nc

# Rename files
rename "s/.nc_day//g" rcp85_MPI_RCA_TM_daily_2011-2100_??.nc_day??.nc

# Copy current and future climate files (to be adjusted by the 3DBC R-scripts)
cd /lustre/storeB/users/andreasd/postclim/3DBC/tas/CurC/
cp /lustre/storeB/users/andreasd/postclim/3DBC/tas/Cur/hist_MPI_RCA_TM_daily_1971-2010_????.nc .

cd /lustre/storeB/users/andreasd/postclim/3DBC/tas/FutC/
cp /lustre/storeB/users/andreasd/postclim/3DBC/tas/Fut/rcp85_MPI_RCA_TM_daily_2011-2100_????.nc .
