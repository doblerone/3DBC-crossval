...
## for ((d=1;d<366;d=d+6)); do let e=d+5; sed "s/START/$d/g; s/END/$e/g" 3DBC_tas_EC-EARTH_HIRHAM_template.R > 3DBC_tas_EC-EARTH_HIRHAM_d${d}.R; done
...
for (d in START:END)
{
  ...
  nc <- nc_open(paste("/lustre/storeB/users/andreasd/postclim/3DBC/tas/Cur/hist_EC-EARTH_HIRHAM_TM_daily_1971-2010_",daystring,".nc",sep=""))
  ...
  nc <- nc_open(paste("/lustre/storeB/users/andreasd/postclim/3DBC/tas/Fut/rcp85_EC-EARTH_HIRHAM_TM_daily_2011-2100_",daystring,".nc",sep=""))
  FutA <- ncvar_get(nc,"air_temperature__map_rcp85_daily")/10-273.15
  nc_close(nc)
  #reading done
  ...
  nc <- nc_open(paste("/lustre/storeB/users/andreasd/postclim/3DBC/tas/CurC/hist_EC-EARTH_HIRHAM_TM_daily_1971-2010_",daystring,".nc",sep=""),write=TRUE)
  ncvar_put(nc,"air_temperature__map_hist_daily",OUT)
  nc_close(nc)
  
  OUT <- (FutCA+273.15)*100
  nc <- nc_open(paste("/lustre/storeB/users/andreasd/postclim/3DBC/tas/FutC/rcp85_EC-EARTH_HIRHAM_TM_daily_2011-2100_",daystring,".nc",sep=""),write=TRUE)
  ncvar_put(nc,"air_temperature__map_rcp85_daily",OUT)
  nc_close(nc)
