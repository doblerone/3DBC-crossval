...
## for ((d=1;d<366;d=d+6)); do let e=d+5; sed "s/START/$d/g; s/END/$e/g" 3DBC_pr_EC-EARTH_HIRHAM_template.R > 3DBC_pr_EC-EARTH_HIRHAM_d${d}.R; done
...
for (d in START:END)
{
  ...
  nc <- nc_open(paste("/lustre/storeB/users/andreasd/postclim/3DBC/pr/Cur/hist_EC-EARTH_HIRHAM_RR_daily_1971-2010_",daystring,".nc",sep=""))
  ...
  nc <- nc_open(paste("/lustre/storeB/users/andreasd/postclim/3DBC/pr/Fut/rcp85_EC-EARTH_HIRHAM_RR_daily_2011-2100_",daystring,".nc",sep=""))
  FutA <- ncvar_get(nc,"precipitation__map_rcp85_daily")/10
  ...
  OUT <- CurCA
  nc <- nc_open(paste("/lustre/storeB/users/andreasd/postclim/3DBC/pr/CurC/hist_EC-EARTH_HIRHAM_RR_daily_1971-2010_",daystring,".nc",sep=""),write=TRUE)
  ncvar_put(nc,"precipitation__map_hist_daily",OUT)
  nc_close(nc)
  
  OUT <- FutCA
  nc <- nc_open(paste("/lustre/storeB/users/andreasd/postclim/3DBC/pr/FutC/rcp85_EC-EARTH_HIRHAM_RR_daily_2011-2100_",daystring,".nc",sep=""),write=TRUE)
  ncvar_put(nc,"precipitation__map_rcp85_daily",OUT)
  nc_close(nc)
